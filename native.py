...
resize_y = 600
image_zoom = 10
pixel_to_mm_10x = 1.1    # 11 pixels = 1 mm
pixel_to_mm_25x = 2.7    # 27 pixels = 1 mm
deleted_circles = 0
...
def save_found_circles_data(found_circles, window):
    ...
    final_img_path = os.getcwd() + '/Results/Native/' + img_path.split('/')[-1]
    ...
    found_circle_data += ("NATIVE;" + img_path.split('%')[0].split(' ')[1] + ";"
                            + str(image_zoom) + ";"
                            + img_path + ";" + str(len(found_circles))
...
def reprocess_image():
    ...
    image = cv2.cvtColor(blurred_img, cv2.COLOR_GRAY2BGR)
...
def save_error(found_circles):
    ...
    global deleted_circles
    current_month_and_year = datetime.date.today().strftime("%m-%Y")
    file_path = os.getcwd() + '/Results/Data/' + 'NATIVE_ERROR_' + current_month_and_year + '.csv'
    file_writing_mode = 'a'    # append mode (writes on existent file)
    found_circle_data = ''
    if not os.path.isfile(file_path):
